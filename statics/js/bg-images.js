...
<div id="bg-overlay" class="bg-imgs bg-mountain"></ div>
...
$(document).ready(function() {
    ...
    $bgBtn.on('click', function(e){
        ...
        if ($el.hasClass('bg-trans')) {
            $target.css('background-image','none').removeClass('bg-imgs');
        ...
        $('<imgs/>').attr('src' , url).on('load', function(){
            $target.css('background-image', 'url("' + url + '")').addClass('bg-imgs');
            $imgHolder.removeClass('disabled');
            $bgBtn.removeClass('active');
            $el.addClass('active');
